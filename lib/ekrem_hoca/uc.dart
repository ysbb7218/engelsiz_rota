...
class _MapPageState extends State<MapPage> {
  ...
  Future<List<LatLng>> getRoute(LatLng start, LatLng end) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Mesafe: ${routeDistanceKm?.toStringAsFixed(2)} km, Süre: ${routeDurationMin?.toStringAsFixed(0)} dk');
        ...
        if (accessibleWaypoints.isNotEmpty) {
          print('${accessibleWaypoints.length} erişilebilir nokta bulundu');
          ...
          print('Optimize edilmiş rota: ${optimizedRoute.length} nokta');
          return optimizedRoute;
        } else {
          print('Erişilebilir nokta bulunamadı, temel rota kullanılıyor');
          return baseRoute;
        }
      } else {
  ...
  Future<List<LatLng>> _findAccessibleWaypointsOnRoute(
    ...
    try {
      ...
      print('📍 Bulunduğumuz konum (0,0): ${start.latitude}, ${start.longitude}');
      ...
      for (final doc in querySnapshot.docs) {
        ...
        // Sadece hedef ile aynı bölgedeki noktaları değerlendir
        if (markerQuadrant != targetDirection) {
          differentQuadrantCount++;
          print('❌ Farklı bölge ($markerQuadrant): ${marker.type} - ${markerLatLng.latitude.toStringAsFixed(6)}, ${markerLatLng.longitude.toStringAsFixed(6)}');
          continue; // Bu noktayı dahil etme
        }
        
        sameQuadrantCount++;
        print('✅ Aynı bölge ($targetDirection): ${marker.type} - ${markerLatLng.latitude.toStringAsFixed(6)}, ${markerLatLng.longitude.toStringAsFixed(6)}');
        ...
        // Rota üzerinde çok uzak noktaları filtrele (500m)
        if (minDistanceToRoute > 500) {
          print('   ⚠️ Rotaya uzak (>500m): ${minDistanceToRoute.toStringAsFixed(0)}m');
          continue;
        }
        ...
        // Puanlama sistemi (sadece aynı bölgedeki noktalar için)
        double score = 0.0;

        // 1. Rota yakınlığı (0-100 puan)
        score += (500 - minDistanceToRoute) / 500 * 100;

        // 2. Sapma mesafesi (0-100 puan) - sapma az ise yüksek puan
        if (detourDistance <= 0) {
          score += 100; // Sapma yok, maksimum puan
        } else if (detourDistance <= 1000) {
          score += (1000 - detourDistance) / 1000 * 100; // 0-1000m arası
        } else {
          score += 0; // 1000m'den fazla sapma
        }

        // 3. Erişilebilirlik türü puanı (0-50 puan)
        if (selectedProfile == 'wheelchair') {
          if (marker.type == 'rampa') score += 50;
          else if (marker.type == 'asansör') score += 40;
        ...
        // 4. Bölge önceliği (100 puan - zaten aynı bölgede)
        score += 100; // Hedef yönü ile aynı bölge

        scoredPoints.add(MapEntry(marker, score));
        
        print('   📊 Puan: ${score.toStringAsFixed(1)} - Rota mesafesi: ${minDistanceToRoute.toStringAsFixed(0)}m, Sapma: ${detourDistance.toStringAsFixed(0)}m');
      }

      print('📈 Bölge dağılımı:');
      print('   🎯 Hedef bölgesi ($targetDirection): $sameQuadrantCount nokta');
      print('   ❌ Diğer bölgeler: $differentQuadrantCount nokta (dahil edilmedi)');

      if (scoredPoints.isEmpty) {
        print('⚠️ Hedef bölgesinde uygun erişilebilir nokta bulunamadı!');
        return [];
      }
      ...
      // En yüksek puanlı 5 noktayı al
      final topPoints = scoredPoints.take(5).map((e) => LatLng(e.key.latitude, e.key.longitude)).toList();

      print('🎯 Seçilen ${topPoints.length} nokta (sadece $targetDirection bölgesi):');
  ...
  Future<List<LatLng>> _optimizeRouteWithWaypoints(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Optimizasyon API hatası: ${response.statusCode}');
  ...
  Future<void> drawRoute() async {
    ...
    try {
      ...
      _showMessage('🎯 Hedef yönü: $targetDirection', isError: false);
