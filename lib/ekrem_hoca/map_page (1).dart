...
class _MapPageState extends State<MapPage> {
  ...
  Future<void> _announceRouteProgress() async {
    ...
    // Rota üzerindeki ilk ve son noktaları kontrol et
    if (routePoints.length >= 2) {
      ...
      if (distanceToStart < 20) {
        await _speak(
          'Rota başlangıcına yaklaştınız. Yönlendirmeyi takip edin.',
        );
      } else if (distanceToEnd < 10 && !_hasAnnouncedDestinationReached) {
        await _speak('Hedef noktaya ulaştınız! Yolculuğunuz tamamlandı.');
        _hasAnnouncedDestinationReached = true;
      } else if (distanceToEnd < 20) {
        await _speak('Hedef noktaya yaklaştınız. Yolculuğunuz tamamlanıyor.');
      } else if (distanceToEnd < 100) {
        await _speak(
          'Hedef noktaya yaklaşıyorsunuz. ${distanceToEnd.toInt()} metre kaldı.',
        );
      }
    }

    // Rota üzerindeki dönüş noktalarını kontrol et
      ...
      for (int i = 1; i < routePoints.length - 1; i++) {
        ...
        if (distanceToTurn < 30) {
          ...
          if (bearingDiff > 45) {
            String direction = '';
            if (bearingDiff > 90) {
              direction = bearingDiff > 135 ? 'geri dönüş' : 'keskin dönüş';
            } else {
              direction = 'hafif dönüş';
            }
            await _speak('$direction yaklaşıyor. Hazırlanın.');
            break; // Sadece en yakın dönüşü duyur
          }
        }
      }
    }
