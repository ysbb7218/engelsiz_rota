...
class _MapPageState extends State<MapPage> {
  ...
  bool _isStartPointManuallySet = false; // New flag to track manual start point
  ...
  void initState() {
    ...
    locationTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      if (!_isStartPointManuallySet) {
        // Only update if start point not manually set
  ...
  void centerOnCurrentLocation(LatLng pos) async {
    setState(() {
      startPoint = pos;
      _isStartPointManuallySet =
          false; // Reset manual flag when using current location
    });
  ...
  void clearRoute() async {
    ...
    if (_currentLocationSymbol != null && _controller != null) {
      try {
    ...
    setState(() {
      ...
      _isStartPointManuallySet = false; // Reset manual flag on route clear
    });
  }
  ...
  Future<void> _showLongPressOptions(
    maplibre.Point point,
    maplibre.LatLng latLng,
  ) async {
    ...
    if (_controller == null || selectedOption == null) return;

    if (selectedOption == 'start') {
      setState(() {
        startPoint = LatLng(latLng.latitude, latLng.longitude);
        _isStartPointManuallySet = true; // Set flag to true
      });
      if (_currentLocationSymbol != null) {
        await _controller!.updateSymbol(
          _currentLocationSymbol!,
      ...
      body: Stack(
        children: [
          maplibre.MaplibreMap(
            ...
            onMapLongClick: _showLongPressOptions,
          ),
          Column(
