...
class _MapPageState extends State<MapPage> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (accessibleWaypoints.isNotEmpty) {
          ...
          // 3. Yaya geçidi kontrolü ve özel optimizasyon
          bool hasPedestrianCrossings = await _checkForPedestrianCrossings(accessibleWaypoints);
          
          if (hasPedestrianCrossings) {
            print('🚶 Yaya geçidi tespit edildi, özel optimizasyon uygulanıyor...');
            List<LatLng> optimizedRoute = await _optimizePedestrianCrossingRoute(
              start, end, baseRoute, accessibleWaypoints
            );
            
            if (optimizedRoute.isNotEmpty) {
              print('🚀 Yaya geçidi optimize edilmiş rota hazır: ${optimizedRoute.length} nokta');
              return optimizedRoute;
            }
          }
          
          // 4. Normal rota optimizasyonu yap
  ...
  // Yaya geçidi rotaları için özel optimizasyon
  Future<List<LatLng>> _optimizePedestrianCrossingRoute(
    LatLng start, LatLng end, List<LatLng> baseRoute, List<LatLng> waypoints
  ) async {
    if (waypoints.isEmpty) return baseRoute;
    
    print('=== YAYA GEÇİDİ ROTA OPTİMİZASYONU ===');
    
    // Yaya geçidi türündeki waypoint'leri filtrele
    List<LatLng> pedestrianCrossings = [];
    for (final wp in waypoints) {
      // Firestore'dan bu noktanın türünü kontrol et
      final markerType = await _getMarkerTypeAtLocation(wp);
      if (markerType == 'yaya_gecidi') {
        pedestrianCrossings.add(wp);
      }
    }
    
    if (pedestrianCrossings.length < 2) {
      print('ℹ️ Yeterli yaya geçidi bulunamadı, normal optimizasyon kullanılıyor');
      return await _optimizeRouteWithWaypoints(start, end, baseRoute, waypoints);
    }
    
    print('🚶 ${pedestrianCrossings.length} yaya geçidi bulundu, özel optimizasyon uygulanıyor');
    
    // Yaya geçitlerini rota boyunca sırala
    List<LatLng> orderedCrossings = _orderPedestrianCrossingsAlongRoute(start, end, baseRoute, pedestrianCrossings);
    
    print('🔄 Sıralanmış yaya geçitleri:');
    for (int i = 0; i < orderedCrossings.length; i++) {
      final crossing = orderedCrossings[i];
      final distance = _calculateDistance(start, crossing);
      final bearing = _getBearing(start, crossing);
      print('   ${i + 1}. ${crossing.latitude.toStringAsFixed(6)}, ${crossing.longitude.toStringAsFixed(6)}');
      print('      📏 Mesafe: ${distance.toStringAsFixed(0)}m, 🧭 Yön: ${bearing.toStringAsFixed(1)}°');
    }
    
    // Yaya geçitleri arasında düz rotalar oluştur
    List<LatLng> optimizedRoute = await _createSmoothRouteBetweenCrossings(
      start, end, orderedCrossings
    );
    
    if (optimizedRoute.isNotEmpty) {
      print('✅ Yaya geçidi rotası başarıyla optimize edildi');
      print('📏 Rota nokta sayısı: ${optimizedRoute.length}');
      return optimizedRoute;
    } else {
      print('⚠️ Yaya geçidi rotası oluşturulamadı, normal optimizasyon kullanılıyor');
      return await _optimizeRouteWithWaypoints(start, end, baseRoute, waypoints);
    }
  }

  // Yaya geçitlerini rota boyunca sırala
  List<LatLng> _orderPedestrianCrossingsAlongRoute(
    LatLng start, LatLng end, List<LatLng> baseRoute, List<LatLng> crossings
  ) {
    if (crossings.length <= 1) return crossings;
    
    // Her yaya geçidi için rota üzerindeki pozisyonu hesapla
    List<MapEntry<LatLng, double>> crossingPositions = [];
    
    for (final crossing in crossings) {
      // Rota üzerindeki en yakın noktaya olan mesafeyi ve indeksi bul
      double minDistance = double.infinity;
      int closestIndex = -1;
      
      for (int i = 0; i < baseRoute.length; i++) {
        final distance = _calculateDistance(crossing, baseRoute[i]);
      ...
      if (closestIndex != -1) {
        // Rota ilerleme oranı (0.0 = başlangıç, 1.0 = hedef)
        final routeProgress = closestIndex / (baseRoute.length - 1);
        crossingPositions.add(MapEntry(crossing, routeProgress));
      }
    }
    
    // Rota üzerindeki pozisyona göre sırala
    crossingPositions.sort((a, b) => a.value.compareTo(b.value));
    
    return crossingPositions.map((e) => e.key).toList();
  }

  // Yaya geçitleri arasında düz rota oluştur
  Future<List<LatLng>> _createSmoothRouteBetweenCrossings(
    LatLng start, LatLng end, List<LatLng> crossings
  ) async {
    try {
      print('🔄 Yaya geçitleri arasında düz rota oluşturuluyor...');
      
      // ORS API'ye yaya geçitleriyle birlikte istek gönder
      final String url = 'https://api.openrouteservice.org/v2/directions/foot-walking/geojson';
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          'coordinates': [
            [start.longitude, start.latitude],
            ...crossings.map((crossing) => [crossing.longitude, crossing.latitude]),
            [end.longitude, end.latitude],
          ],
          'elevation': false,
          'preference': 'fastest',
          'continue_straight': true, // Yaya geçitleri arasında düz git
          'avoid_features': ['highways'], // Ana yollardan kaçın
          'vehicle_type': 'foot', // Yaya rotası
        }),
      );
      ...
      if (response.statusCode == 200) {
        ...
        // Yaya geçidi rotası için özel yumuşatma
        optimizedRoute = _smoothPedestrianRoute(optimizedRoute, crossings);
        ...
        print('✅ Yaya geçidi rotası başarıyla oluşturuldu');
        print('📏 Yeni mesafe: ${routeDistanceKm?.toStringAsFixed(2)} km, Süre: ${routeDurationMin?.toStringAsFixed(0)} dk');
        
        return optimizedRoute;
      } else {
        print('⚠️ Yaya geçidi rota API hatası: ${response.statusCode}');
        return [];
      }
    } catch (e) {
      print('❌ Yaya geçidi rota oluşturma hatası: $e');
      return [];
    }
  }

  // Yaya geçidi rotası için özel yumuşatma
  List<LatLng> _smoothPedestrianRoute(List<LatLng> route, List<LatLng> crossings) {
    if (route.length <= 3) return route;
    
    print('=== YAYA GEÇİDİ ROTA YUMUŞATMA ===');
    ...
    for (int i = 1; i < route.length - 1; i++) {
      ...
      // Bu nokta bir yaya geçidi mi kontrol et
      bool isPedestrianCrossing = crossings.any((crossing) => 
        _calculateDistance(current, crossing) < 20 // 20m yakınlık
      );
      
      if (isPedestrianCrossing) {
        // Yaya geçidi noktalarını olduğu gibi bırak
        smoothedRoute.add(current);
        print('🚶 Yaya geçidi noktası korundu: ${current.latitude.toStringAsFixed(6)}, ${current.longitude.toStringAsFixed(6)}');
      } else {
        // Normal noktalar için yumuşatma uygula
        final angle = _calculateAngle(prev, current, next);
        
        if (angle < 60) { // 60 dereceden küçük açı
          // Orta nokta ekle
          final midPoint = LatLng(
            (prev.latitude + next.latitude) / 2,
            (prev.longitude + next.longitude) / 2,
    ...
    print('✅ Yaya geçidi rota yumuşatma tamamlandı');
    print('📏 Giriş: ${route.length} nokta, Çıkış: ${smoothedRoute.length} nokta');
    
    return smoothedRoute;
  }

  // Belirli konumdaki marker türünü al
  Future<String?> _getMarkerTypeAtLocation(LatLng location) async {
    try {
      final querySnapshot = await FirebaseFirestore.instance
          .collection('markers')
          .where('latitude', isGreaterThan: location.latitude - 0.001)
          .where('latitude', isLessThan: location.latitude + 0.001)
          .where('longitude', isGreaterThan: location.longitude - 0.001)
          .where('longitude', isLessThan: location.longitude + 0.001)
          .get();
      
      if (querySnapshot.docs.isNotEmpty) {
        final marker = MarkerModel.fromMap(querySnapshot.docs.first.data());
        return marker.type;
      }
      return null;
    } catch (e) {
      print('❌ Marker türü alma hatası: $e');
      return null;
    }
  }

  // Waypoint'ler arasında yaya geçidi olup olmadığını kontrol et
  Future<bool> _checkForPedestrianCrossings(List<LatLng> waypoints) async {
    if (waypoints.isEmpty) return false;
    
    print('🔍 Yaya geçidi kontrolü yapılıyor...');
    
    int pedestrianCrossingCount = 0;
    
    for (final wp in waypoints) {
      final markerType = await _getMarkerTypeAtLocation(wp);
      if (markerType == 'yaya_gecidi') {
        pedestrianCrossingCount++;
        print('🚶 Yaya geçidi bulundu: ${wp.latitude.toStringAsFixed(6)}, ${wp.longitude.toStringAsFixed(6)}');
      }
    }
    
    print('📊 Toplam yaya geçidi sayısı: $pedestrianCrossingCount');
    
    // En az 2 yaya geçidi varsa özel optimizasyon uygula
    return pedestrianCrossingCount >= 2;
  }
}
